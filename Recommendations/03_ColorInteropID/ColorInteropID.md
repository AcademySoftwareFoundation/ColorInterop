...
Unfortunately, it has been difficult to get the industry to coalesce around standard names for color spaces. The ACES project made the most concerted attempt at trying to make this happen but, despite their best efforts, even standard ACES color spaces are often referred to by a variety of different names (e.g., "ACES2065-1" vs. "lin_ap0"). Often, VFX/animation studios or applications have long-standing practices concerning how to name color spaces and it is very difficult to make changes.
...
In order to keep the requirements minimal, and because the user-facing name is just a recommendation that may be overridden, only the interop ID is designed to be stored in file formats.

However, the Color Interop Forum only develops recommendations for the most common color spaces and there is a need to be able to generate an ID string for any possible color space. To avoid the problem of different groups trying to use the same string for different color spaces, a "namespace" is used along with the base color space name. That way, people may generate IDs for unique color spaces with minimal risk of colliding with IDs generated by others.
...
The color interop ID is structured as a base name with up to two namespace prefixes:
`<OUTER_NAMESPACE>:<INNER_NAMESPACE>:<BASE>`

The structure of the ID has three modes, designed for various use-cases.

#### Mode 1: Color Interop Forum published Recommendations

If the base string is one of the Color Interop Forum published IDs, the namespace and separator are not used. For example: `lin_rec709_scene`. This reduces the length of the ID for the common color spaces that represent the vast majority of use-cases. For other color spaces, that have not been published as part of an official CIF Recommendation, the namespace is mandatory so one of the other modes must be used.

#### Mode 2: IDs generated by organizations

Organizations generating IDs must use at least one namespace. For example: `my-studio:my-colorspace`. 

Organizations may use a two-level namespace, if they desire. This may be useful in connection with the ID resolution process described below. For example: `my-show:my-studio:my-colorspace`.

Organizations should typically avoid using base names that are used by others to avoid collisions, as described in the ID resolution process below.

#### Mode 3: On-demand ID generation

Sometimes it is necessary for software to generate an ID on demand to describe a color space that does not already have an ID. This form of ID is essentially a "local" reference to a color space in a specific color configuration. In this mode, the namespace has two parts, the name of the configuration, followed by the special namespace value `local`, as follows:
`<CONFIG-NAME>:local:<BASE>`

This makes it possible for applications to generate interop IDs in a way that minimizes the possibility of collision with other IDs since they are only used locally within a specific configuration.

In the case of OCIO, it is important for config authors to set the `name` attribute (within the config file) to something that is unlikely to conflict with other configs, since that is what will be used as the outer namespace.

Annex B contains a more precise, parser oriented, description of the ID syntax. 

#### ID Resolution Process

When searching for an ID, a color management system is allowed to drop the leftmost namespace (and one separator) and search again for the resulting string. For example, if the ID is `ocio:arrilogc4_awg4_scene`, if that ID is not found, the `ocio:` may be dropped and `arrilogc4_awg4_scene` could be searched for. With a two-level namespace (i.e., there are two `:` separators in the ID), only the leftmost one may be dropped.

If that does not return a result, the color management system should check if it's an "On-demand" mode ID by checking that it is a two-level namespace where the inner namespace is `local`. If the outer namespace matches the name of the color configuration currently in use, then the base name is searched for.

For example, for the ID `show1-config:local:srgb`, the color management system checks the name of the configuration and, only if it is `show1-config`, it then searches that config for the base name `srgb`. This example would not match just `srgb` in a different color configuration.

This search process allows IDs to be specific but have a built-in fallback. Here are some examples:
...
* An animation studio could use the ID `show_name:studio_name:our-rendering-space` and if the color management system was unable to match the full ID, it would try `studio_name:our-rendering-space`. In other words, a studio-wide default could be substituted for a show-specific variant. However, this would not match only the base `our-rendering-space`.
...
#### Allowed Characters for ID Strings

To ensure portability and robustness, the following restrictions are placed on the characters used in an interop ID:

1. It may only use lower-case ASCII characters: 0-9, a-z, and the following characters (no spaces):  
   `. - _ ~ / * # % ^ + ( ) [ ] |`
2. A colon character `:` may only be used as a namespace separator. There may be at most two in an ID.
...
When generating interop IDs, it is important to avoid using the same string that may be used by others. A Wiki has been set up on the Color Interop Forum GitHub site to allow entities to reserve namespaces or IDs to avoid collisions with other ID creators. 

For example, a namespace consisting of "ocio", or any string beginning with "ocio", is listed on the Wiki as reserved for use by the OCIO project maintainers.

Please see Annex A for more information about the interop ID registry.

#### Uniqueness of the Interop ID

A color interop ID is intended to be an unambiguous identifier that may be used to match up color spaces that are functionally equivalent. It is not a "unique ID" in the sense that it is not intended to be unique to a specific color space implementation, for example, in a specific OCIO config. If color spaces in two configs share an interop ID, it does not imply that a hash function such as a config or processor ID of the two would also be identical. In the case of OCIO color spaces, for example, the aliases or other attributes may differ, a different reference space may be used, one may use an analytic log or exponent transform and the other may use a LUT, or other such differences may be present.
...
Searching uses the usual `Config::getColorSpace` function. For that reason, it is essential that config authors set the interop ID as an alias on the color space. (The `ociocheck` command-line tool verifies that this is the case.)
...
If no interop ID is found in step 1, the application may want to provide special handling for the utility interop IDs `data` and `unknown` based on the descriptions in the "Color Space Encodings for Texture Assets and CG Rendering" recommendation.
...
The grammar is expressed in ABNF notation per [RFC 5234](https://www.rfc-editor.org/rfc/rfc5234). It operates on the ASCII character set. Strings containing non-ASCII characters or disallowed characters must first be sanitized per Annex C before use in an interop ID.
...
All implementations of interop IDs must use the method described here. This is because the sanitization happens during both the ID generation and search process and both must use the identical algorithm in order for the search to succeed.

Here is Python code to illustrate the algorithm:
